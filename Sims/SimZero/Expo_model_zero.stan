...
real log_rho;                  # Site-level log(Expected detection rate)
real<lower=0> rho;             # Site-level detection rate
...
log_p[1] = exponential_lcdf(tau[1] | rho);
log_mu_ab[1] = log_lambda + log_p[1];
for (i in 2:n_ints) {
  log_p[i] = log_diff_exp(exponential_lccdf(tau[i-1] | rho),
    exponential_lccdf(tau[i] | rho));
