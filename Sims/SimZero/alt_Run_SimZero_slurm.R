...
for(indx in indices[j:(j+bundle-1)]){
  ...
  m = stan_model(paste0("alt_",simtable$modelname[indx],".stan"), auto_write=rstan_options("auto_write" = TRUE))
  ...
  save(list=as.character(simtable$modeloutput[indx]), file=paste0("Sim_",Rep,"/alt_",simtable$fname[indx],".Rdata"))
