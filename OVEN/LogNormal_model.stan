...
for (s in 1:n_sites) {
  log_p[s,1] = lognormal_lcdf(tau[1] | mu_det[s],sigma_det);
  log_mu_ab[ii[s]+1] = log_lambda[s] + log_p[s,1];
  for (i in 2:n_ints) {
    log_p[s,i] = log_diff_exp(lognormal_lccdf(tau[i-1] | mu_det[s],sigma_det),
...
dev1 = 0;
for (s in 1:n_sites) {
  unobserved[s] = poisson_log_rng(log_lambda[s] + lognormal_lccdf(tau[n_ints] | mu_det[s], sigma_det));
