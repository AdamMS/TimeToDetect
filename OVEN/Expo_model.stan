...
int ia[n_sites,n_ra];      # IDs what abundance random effect levels go with each site survey
int id[n_sites];           # IDs what detection random effect level goes with each site survey  
...
transformed data {
int ii[n_sites];         # An index identifying where in the vectorized data each site begins (minus 1)
int yv[n_sites*n_ints];  # Vectorized data
int obsN[n_sites];       # Total count at each site across all intervals
...
vector[n_sites] log_lambda;     # Site-level log(Expected abundance)
vector[n_sites] log_rho;        # Site-level log(Expected detection rate)
vector<lower=0>[n_sites] rho;   # Site-level detection rate
...
vector[n_sites] lambda;         # Site-level Expected abundance
...
for (s in 1:n_sites) {
  log_p[s,1] = exponential_lcdf(tau[1] | rho[s]);
  log_mu_ab[ii[s]+1] = log_lambda[s]+log_p[s,1];
  for (i in 2:n_ints) {
    log_p[s,i] = log_diff_exp(exponential_lccdf(tau[i-1] | rho[s]),
...
dev1 = 0;
for (s in 1:n_sites) {
  unobserved[s] = poisson_log_rng(log_lambda[s] + exponential_lccdf(tau[n_ints] | rho[s]));
