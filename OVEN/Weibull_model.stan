...
for (s in 1:n_sites) {
  log_p[s,1] <- weibull_cdf_log(tau[1],shape_k,invrate[s]);
  log_mu_ab[ii[s]+1] <- log_lambda[s] + log_p[s,1];
  for (i in 2:n_ints) {
    log_p[s,i] <- log_diff_exp(weibull_cdf_log(tau[i],shape_k,invrate[s]),
                               weibull_cdf_log(tau[i-1],shape_k,invrate[s]));
