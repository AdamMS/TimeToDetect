...
for (s in 1:n_sites) {
  log_p[s,1] = log(gamma * weibull_cdf(tau[1],shape_k,invrate[s]) + 1 - gamma);
  log_mu_ab[ii[s]+1] = log_lambda[s] + log_p[s,1];
  for (i in 2:n_ints) {
    log_p[s,i] = log(gamma) + log_diff_exp(weibull_lcdf(tau[i] | shape_k,invrate[s]),
...
dev1 = 0;
for (s in 1:n_sites) {
  unobserved[s] = poisson_log_rng(log_lambda[s] + log(gamma) + weibull_lccdf(tau[n_ints] | shape_k, invrate[s]));
