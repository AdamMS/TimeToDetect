...
for (s in 1:n_sites) {
  log_p[s,1] <- log(gamma * gamma_cdf(tau[1],alpha,beta[s]) + 1 - gamma);
  log_mu_ab[ii[s]+1] <- log_lambda[s] + log_p[s,1];
  for (i in 2:n_ints) {
    log_p[s,i] <- log(gamma) + log_diff_exp(gamma_cdf_log(tau[i],alpha,beta[s]),
                                            gamma_cdf_log(tau[i-1],alpha,beta[s]));
