...
We analyze times to first detection as time-to-event data in a parametric survival analysis, defining a continuous random variable $T$ for each individual's time to first detection with a probability density function (pdf) $f_T(t)$ and cumulative distribution function (cdf) $F_T(t)$.  
...
Like the gamma TTDD, Weibull and lognormal TTDDs offer the flexibility of a two-parameter form and allow for rates to either increase or decrease during the survey; however, the shapes of these distributions will differ, most notably in their tail probabilities.
All three could be reasonable TTDD choices if, for instance, we believe detection rates vary from individual to individual.
In thats case, marginal detection rates decrease over time as individuals with higher detection rates are removed from the local population.
