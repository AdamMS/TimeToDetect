...
Each stand had up to four \jarad{I thought it was 3} sites with sufficient geographical distance between sites to reduce or eliminate overlapping territories.
...
\jarad{did we use unlimited distance point counts or did we use $<$100m?}
...
p_{s1} &= \left[(1-\gamma) + \gamma\Fm(C_1|\alpha,\varphi_s)\right]/F_T(C_I|\alpha,\varphi_s) \nonumber\\
p_{si} &= \gamma\left[\Fm(C_i|\alpha,\varphi_s) - \Fm(C_{i-1}|\alpha,\varphi_s)\right]/F_T(C_I|\alpha,\varphi_s) \nonumber\\
...
We chose true parameter values (see Supplement \jarad{ADAM: what section in the supplement}) to mimic values from the Ovenbird danalysis see Section \ref{sec:ovenbirds}. In particular, we set parameters such that (i) the overall expected detection probability was 0.80, (ii) for mixture datasets, $\gamma = 0.65$ (meaning 35\% of individuals were immediately detected), (iii) in nonpeaked models, 70\% of \textit{detected} individuals were observed during the first two minutes, and (iv) in peaked models, the detection mode for `hard to detect' individuals occured at 5 minutes.
\jarad{ADAM: How many years and sites?}
...
Patterns in posterior estimates of site-specific detection probabilities, $\pdet_s$, with respect to mixture and family TTDD forms were the same as in the previous simulation studies -- the inclusion of explanatory variables did not make models more robust to violations of mixture- and constant-detection assumptions, see Figures \ref{pdet_cater_correct} and \ref{pdet_cater_family} in the Supplement.  
...
\jarad{need to define what we mean by $\pdet$ here.}

\jarad{where's the supplement? I think we should just refer to to the Figures on the supplement just like we refer to papers in the main manuscript and then interpret them.}
...
We associated random effects with each survey year and each stand \jarad{did we define stand previously?}.
...
\jarad{ADAM: please change facet names to use Greek letters for $\gamma$ and $\alpha$ and the standard notation we have been using for $\pdet$.}
...
% Really, I have totally ignored the discussion of the true value of p, which I have tended to simulate at a fairly high value
...
An assumption of constant detection constrains uncertainty in the extrapolated tail but at the cost of potential bias.
When we allow for non-constant detection, estimation of the tail probability becomes so variable that we were only able to estimate Ovenbird abundance within an order of magnitude.
...
Without extending the observation period, an alternate to removal sampling is to record complete detection records (all detections for every individual) instead of just the first \citep{Alldredge2007}; however, this may not be feasible in studies like MNFB, where many species are observed.  
...
%\adam{I'm  beginning to think this may have too-numerous antecedents to cite... in Amundson and in Reidy, the earliest reference is Marsh and Sinclair 1989.  See also McCallum 2005.}.  
...
\adam{Two topics I have omitted from the Discussion (which reflects my own judgment, but you may disagree:\\
\begin{enumerate}
\item DIC and posterior predictive checks
\item Selection of priors
