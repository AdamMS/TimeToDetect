...
To accommodate this empirical observation, many models of interval-censored removal times define a TTDD that includes a mixture component to increase the probability of observing individuals in the first interval \citep{Farnsworth2002, Royle2004Generalized, Farnsworth2005, Etterson2009, Reidy2011}.
We define a TTDD with mixing parameter $\gamma\in[0,1]$, a point-mass during the first observation interval, and a continuous-time distribution $\Fm(t)$ such that $F_T(t) = (1-\gamma) + \gamma \Fm(t)$ for $t>0$.
%\adam{This formulation became messy, but I think we can omit it.} The updated interval-specific detection probabilities become: $p_{s1} = [1 - \gamma + \gamma\Fm(C_1)]/F_T(C_I)$ and $p_{si} = \gamma[\Fm(C_i)-\Fm(C_{i-1})]/F_T(C_I)$, for $i=2\dots I$.
When including this parameter and these modifications, we refer to the model as a ``mixture'' model. 
If $\gamma=1$, then the non-mixture model is recovered.
