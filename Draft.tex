...
Assuming all individuals are observed according to a common time-to-detection distribution (TTDD), then $T_{sb}$ is a random variable with cumulative distribution function (cdf) $F_T(t)$ and probability density function (pdf) $f_T(t)$.
If the times to first detection are truncated at $C$, due to a finite survey length of $C$, the probability of detection for each individual present at the survey site is $\pdet=F_T(C)$ and the conditional distribution of observed detection times has pdf $f_{T|det}(t)= f_T(t)/F_T(C)$ for $0<t<C$ and cdf $F_{T|det}(t) = \int_0^t f_{T|det}(x) dx$. 
Denoting $N_{s}\ge n_s^{(obs)}$ as the total number of individuals present at a survey and assuming detection times for individuals are independent, we have $n_{s}^{(obs)} \ind \mbox{Binomial}\left(N_{s}, \pdet\right)$ and $T_{sb|det} \ind f_{T|det}(t_{sb})$
where $T_{sb|det}$ is observed only for those $n_s^{(obs)}$ individuals whose observed time to first detection is less than $C$, i.e. $t_{sb}<C$. 
The instantaneous detection rate, or hazard function, is $h(t) = f_T(t) / [1-F_T(t)]$.

\adam{Thoughts on the above paragraph, recognizing that it is one of the most finicky in the whole paper: (i) Do the first few sentences make clear enough the distinction between $t \in (0, \infty)$ and $t|det \in (0,C)$?  (ii) Is $T_{sb|det} \ind f_{T|det}(t_{sb})$ correct notation? It feels odd to me to have a pdf on the right side of that tilde, but maybe it's just inexperience.  (iii) You originally wrote ``all individuals are observed according to a common rate".  Due to heterogeneity, I replaced ``rate" with ``time-to-detection distribution", though I'd be fine with ``hazard function".  But, in truth, even that's not true -- due to heterogeneity and the mixture formulation.  What we're actually doing is modeling all individual birds from the marginal TTDD across group-specific hazard rates... rather a mouthful.  (iv) I liked the structure that was imparted in the earlier draft by having a data model, abundance model, and detection model.}
...
To facilitate the later inclusion of fixed and random effects, we use the following rate-based parameterizations: $T\sim \Exp(\varphi), E[T]=1/\varphi$; $T\sim \Ga(\alpha,\varphi), E[T] = \alpha/\varphi$; $T\sim \We(\alpha,\varphi), E[T]=\mathrm{\Gamma}(1+1/\alpha)/\varphi$; and $T\sim \LN(\varphi,\alpha), E[T] = e^{\alpha/2}/\varphi$.  \adam{Expressing the LN in rate parameterization is a little unconventional... We may prefer to be more explicit about the relationship between $\mu$ and $\varphi$ by writing: $T \sim LN(-\log(\varphi), \alpha)$.}
The exponential distribution is a special case of both the gamma and Weibull distributions when $\alpha=1$. 

Given exact times to first detection, we can estimate the parameters in a TTDD and thus estimate $\pdet$ and its uncertainty. 
...
\jarad{Adam: add refs.}
\adam{The underdispersion finding comes from our own modeling of the Ovenbird dataset.  I recall no discussion of underdispersion in the literature.  The primary source for abundance distributions removal models is Etterson.}
...
To accommodate this empirical observation, many models of interval-censored removal times include a parameter to increase the probability of observing individuals in the first interval \citep{Farnsworth2002, Royle2004Generalized, Farnsworth2005, Etterson2009, Reidy2011}.
...
\adam{Three things that we'll probably want to talk through: (i) you've switched the mixture components -- my $\gamma$ has become (1-$\gamma$) in the new nomenclature; (ii) I feel that $f_T(t)$ and $F_T(t)$ should describe the entire TTDD and that a different function nomenclature should be used for the `hard to detect' birds.  (iii) This dates back to my original draft... but this particular form of mixture -- with the point mass -- is really specific to interval-censoring.  It feels incongruous to be mixing a point-mass and a continuous distribution.  However, writing it propoerly depends upon nomenclature decisions.}
...
For ease of reference, the final full model is provided in equation \eqref{eq:model} where the conditioning of the TTDD cdf on $\alpha$ and $\varphi_s$ is made explicit. \adam{Which equation?}
...
We monitored convergence of the MCMC chains using Geweke diagnostics \citep{Geweke1991}. \jarad{Adam: what did you do with the Geweke diagnostics?} \adam{I'm unclear what you intend with this question.  I used them to monitor convergence.  Any time a chains raised a Geweke flag, I re-ran the model.}
...
A peaked TTDD has a mode greater than zero (or $C_1$ for lognormal) while a non-peaked TTDD has a mode of zero (or less than $C_1$). \jarad{Adam: do we want a figure demonstrating this point?}\adam{I don't think a figure is required.}
...
\jarad{Adam: do we want posterior predictive pvalues at all?}\adam{Interesting question.  I don't think they're essential to our model.  I am open to omitting them.  However... Amundson et al. are vague about the posterior p-values -- ``we used Bayesian P values generated from the posterior predictive distributions to assess goodness-of-fit" -- they don't state what aspect of fit they tested -- it looks like maybe they use the model-fit value of $\pdet$ as their statistic?!?  Anyhow, I suspect they misapplied the tool, and I somewhat wanted to counteract that with an example of a better application of it.  Also, I think it's important to show that we checked some measure of fit.}
...
\jarad{Adam: do we want to remove DIC?}\adam{As with posterior predictive checks, I can live without DIC, which has been nothing but headaches.}
...
\adam{Here's a brazen idea... let's omit Sim3.  We can reference it in the Ovenbird analysis, saying `we did a preliminary simulation study of the model with covariates and random effects, and results were consistent with Sim1, Sim2, and the Ovenbird findings...'.  What do you think?}

The previous sections studyied the effect of the time to detection assumption in the context of no explanatory variables.
Here we incorporate fixed and random effects for abundance and detection. 
For each of the 14 TTDDs, we simulated data using the median posterior parameter estimates obtained in the analysis of Ovenbird data in Section \ref{sec:ovenbirds}.
Because the fitted Ovenbird models did not yield peaked distributions, we simulated peaked datasets by: (i) using the same intercepts, shape parameters, and mixing parameters as in Sim1 and Sim2, (ii) using covariate and random effects from the Ovenbird estimates, and (iii) scaling the detection intercept and random effect to achieve true detection probabilities $\approx 0.8$ with a detection mode at 5 minutes.
(See supplement for actual parameter values).
...
Patterns in posterior estimates of $\pdet_s$ with respect to mixture and family TTDD forms were the same in Sim3 as in Sim1 and Sim2 (Figures \ref{pdet_cater_correct} and \ref{pdet_cater_family}.

Possible results to focus upon:\\
\begin{enumerate}
\item Precision of estimates of $\pdet$ relative to Sim1 and Sim2 (they're narrower in Sim3)
% These results indicate that covariates provided significant information for reducing uncertainty.

\item $\beta^A$ estimates are pretty much invariant to choice of TTDD
...
\item $\beta^D$ estimates from exponential mixtures are more precise than for other families.  The coverage situation is unclear based on size of sim study.
...
%Comparing exponential to other TTDD family models (as in Sim2), time-varying mixture TTDD models produced similar posteriors of $\beta^D$ to one another, but posteriors from the exponential mixture model were both location shifted and either narrower (exponential and nonpeaked data) or wider (peaked data).
...
\item DIC can be effective in the same scenarios as Sim1 and Sim2, but it can also be misleading (provide stark example).  This sets the stage for the Ovenbird results.
...
\adam{There is a question as to what Figures/Tables would effectively express the above points.  I think the cleanest approach would be to pick 'representative' datasets and models.  Or, in my first draft, I simply referenced the Ovenbird output to illustrate several of the points...}
...
Exponential & Non-mixture & &  & -1.827 &  \\ 
  ...
  & & Peaked & 0.65 & -2.201 & 1.621 \\ 
   \hline
\end{tabular}
\caption{Table of parameter values used to generate data for Sim1 and Sim2 where $\gamma$ is a mixing parameter for the proportion of `hard to detect' individuals, $\varphi$ is the detection rate parameter, and $\alpha$ is the shape parameter.}
\end{table}
...
Exponential & Non-mixture &  & 0.975 & 0.122 & 0.002 & 0.094 & -1.039 & -1.146 & -0.010 & -0.056 & 0.118 & 0.249 & 0.164 & 0.139 & 0.094 & 0.242 &  &  \\ 
  ...
  Weibull & Non-mixture & Non-peaked & 1.570 & 0.129 & 0.011 & 0.102 & -1.040 & -3.137 & 0.101 & -0.104 & -0.127 & 0.204 & 0.086 & 0.131 & 0.074 & 0.383 &  & 0.387 \\ 
   ...
   \hline
\end{tabular}
\caption{Table of parameter values used to generate data for Sim3 where $\gamma$ is a mixing parameter for the proportion of `hard to detect' individuals, $\beta$'s are fixed effects, $\sigma$'s are random effect standard deviations, $\alpha$ is a shape parameter, and superscripts of $A$ and $D$ indicate abundance and detection parameters, respectively.}
\end{table}
