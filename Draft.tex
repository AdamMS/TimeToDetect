...
A typical assumption in removal sampling is a constant detection rate throughout the observation period, but this assumption is often unjustified \citep{Alldredge2007, LeeMarsden2008}.  
...
We apply the term `mixture model' to any model with a mixture TTDD.
...
Illustration of fitting mixture exponential (left) and mixture gamma (right) time-to-detection distributions (TTDDs) to multinomial removal-sampled observations (center) assuming no survey-level heterogeneity.
The mixture TTDD consists of a continuous TTDD (thick line) plus a mixture component (light gray rectangle), which constitute $\gamma$ and $1-\gamma$ proportions of the population, respectively.
...
\adam{I think we start by describing our joint likelihood: p(counts $| N, \pdet$) and p(times $|$ counts,TTDD) and prior.
We explain that, in isolation, $N$ and $\pdet$ are not identifiable in the first piece, which is why we use the second piece to inform our estimates of $\pdet$.
Then, in the body of section \ref{sec:exact_time}, we introduce a second paragraph which defines our distribution for $\nobs$ and completes the data likelihood.
The discussion of identifiability would travel with the distribution of $\nobs$ and be cleaned up a little.
Perhaps even split the discussion of TTDDs into another subsection.
}
...
\subsection{Exact time to detection}\label{sec:exact_time}
...
The conditional distribution of observed detection times consequently has pdf $f_{T|det}(t)= f_T(t)/F_T(C)$ for $0<t<C$, cdf $F_{T|det}(t) = \int_0^t f_{T|det}(x) dx$, and instantaneous detection rate, or hazard function, $h(t) = f_T(t) / [1-F_T(t)]$. 
...
Like the gamma TTDD, Weibull and lognormal TTDDs offer the flexibility of a two-parameter form and allow rates to either increase or decrease during the survey; however, the shapes of these distributions differ, most notably in tail probability.
...
We employ a log link for $\varphi$, and therefore our model is equivalent to a generalized linear model with a log link on the mean detection time.
...
\caption{
Comparisons of data and models with and without mixture TTDDs.  \textit{Left}: Average posterior median detection probabilities across 100 replicate simulations.  Dashed lines show unbiased estimation.   \textit{Right}: Coverage of 50\% credible intervals across 100 replicate simulations.  Dashed lines depict a 95\% range of observed coverage that is consistent with nominal coverage.  \textit{Rows}: Data simulated from non-mixture (upper) or mixture (lower) TTDDs.  \textit{Columns}:  Data simulated from nonpeaked (left), exponential (center), or peaked (right) TTDDs.  In these simulations, each inference model was only fit to datasets from the same TTDD family (e.g. lognormal to lognormal).
}
...
\caption{
Comparisons of TTDDs across families.  \textit{Left}: Average posterior median detection probabilities across 100 replicate simulations.  Dashed lines show unbiased estimation.   \textit{Right}: Coverage of 50\% credible intervals across 100 replicate simulations.  Dashed lines depict a 95\% range of observed coverage that is consistent with nominal coverage.  \textit{Rows}: Denote the TTDD family used to simulated data.  \textit{Columns}:  Data simulated from nonpeaked (left) or exponential/peaked (right) TTDDs.  In these simulations, we used only mixture datasets and inference models.
}
...
True detection probabilities are shown for comparison (dashed vertical line) and vary by row.
...
Posteriors for abundance fixed and random effects were the same regardless of what TTDD was assumed (Section \ref{S-s:sim3} and figures therein).  
...
When we allow for non-constant detection, bias is less problematic but estimation of the tail probability becomes less certain.
...
To be consistent with the continuous time-to-event approach, distance can be incorporated into the detection model as an event-level modifier as is done in \citeauthor{Borchers2016}.  
