...
In this study, we use interval-censored observations, so in addition to the four above TTDDs (with no mixture component), we consider partial finite-mixtures of a point mass during the first survey interval plus a continuous component from an exponential, gamma, Weibull, or lognormal distribution.  
...
where $F_{T1}$ is the cdf of the continuous component and $\boldsymbol{\theta}$ is a vector of parameters appropriate to $F_{T1}$.  
...
Comparing constant-detection and time-varying models (like in Sim2), time-varying mixture TTDD models generally produced similar posteriors of $\beta^D$ to one another, but posteriors from the exponential mixture model were both location shifted and narrower; they also had lower coverage across all $\beta^D$ parameters (88.5\% vs. 94-97\%).
...
The Ovenbird analysis highlights the ambiguity that emerged from the simulation studies: the results were entirely consistent with data generated from a partial finite-mixture involving a constant-rate $F_{T1}$ TTDD, but they were also consistent with partial finite-mixtures involving a time-varying TTDD.
