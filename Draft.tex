...
One common strategy to deal with data that do not fit a constant-detection assumption is to model the TTDD as a mixture of two distributions --- a continuous-time distribution and an increased detection probability in the initial observation period \citep{Farnsworth2002, Farnsworth2005, EffordDawson2009, Etterson2009, Reidy2011} --- although this is not yet the standard \citep{Solymos2013, Amundson2014, Reidy2016}. 
...
We embed the TTDD in a hierarchical framework for multinomial counts called an N-mixture model \citep{Wyatt2002, Royle2004NMixture}, which references an entirely different `mixture' than the mixture TTDDs in the previous paragraph.
...
We estimate $\pdet$ as the proportion of the TTDD before the end of the observation period $C$, leaving a proportion $1-\pdet$ undetected.
}
\end{figure}
...
Like the gamma TTDD, Weibull and lognormal TTDDs offer the flexibility of a two-parameter form and allow rates to either increase or decrease during the survey; however, the shapes of these distributions will differ, most notably in tail probability.
All three could be reasonable TTDD choices if, for instance, detection rates vary from individual to individual, thus leading to marginal detection rates that decrease over time as individuals with higher detection rates are removed from the population.
...
We refer to models with and without the mixture component as `mixture models' and `non-mixture models', respectively, which are not to be confused with the `N-mixture' model framework we employ.
...
Detection rates $\varphi(t)$ of peaked distributions generally decrease over time, while detection rates of nonpeaked distributions generally increase over time.
...
When we allow for non-constant detection, estimation of the tail probability becomes less certain.
