...
\begin{quote}
\adam{Bolster this with more citations?  \citet{Veech2016} argue for $p>0.5$ in the context of individual-level detection effects (no time-to-detection). 
Plots from \citet{Field2016} suggest uncertainty in $\hat{N}$ increase rapidly as $\pdet$ decreases below 0.40.  Methods include removal, double-observer, distance, and multiple-visit.  In a traditional removal-sampling context, \citet{Davis2016} report accurate $\hat{N}$ at $\pdet >0.40$, though for small abundance ($N<50$) they required $\pdet > 0.7$.}
\end{quote}
