...
vector[n_sites] log_phi;                # Survey-level log(Expected detection rate) -- where rate is a Gamma distribution rate parameter
...
vector[n_ints] cutoffs;                 # Temporary vector to hold cdf at cutoffs (save computation of gamma_cdf)
...
for (s in 1:n_sites) {
  for (i in 1:n_ints) {
    cutoffs[i] = gamma_lcdf(tau[i]|alpha,phi[s]);
  }
  log_p[s,1] = log(gamma * cutoffs[1] + 1 - gamma);
  log_mu_ab[ii[s]+1] = log_lambda[s] + log_p[s,1];
  for (i in 2:n_ints) {
    log_p[s,i] = log(gamma) + log_diff_exp(cutoffs[i], cutoffs[i-1]);
...
real lpn_BK[n_sites,n_ints];         # log(p(n|\phi's,\ksi's))
